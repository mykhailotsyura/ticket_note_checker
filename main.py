...
def find_notask_tickets(xml_data):
    ...
    for ticket in tickets:
        ...
        for note in ticket.findall("./note"):
            jira_search_res = re.search("https?:\/\/" + jira_host + "\/browse\/(" + keys_pattern + ")-[0-9]{3,8}",
                                        note.text)
...
def jira_gettask_bystatus(json_data, task_status):
    if task_status in jira_task_statuses:
        ...
        for issue in json_data['issues']:
            fields = issue['fields']
            if (fields['status']['name'] != task_status and not_cond) or (
                    fields['status']['name'] != task_status and not not_cond):
...
def jira_search_test(search_url, text, login_credentials, processor_function, task_status):
    ...
    search_url += query
    print(search_url)
    headers = {'Authorization': 'Bearer ' + login_credentials['access_token']}
...
webhook_urls = {
    'production': 'https://api.flock.com/hooks/sendMessage/1312c811-ab5f-4843-bf2e-44959d5f46fa',
    'testing': 'https://api.flock.com/hooks/sendMessage/b0a19ed3-79c4-4d32-9772-f19a95ea6455'
}
jira_task_statuses = ['active', 'not_closed', 'closed']
jira_login_credentials = {'access_token': jira_token}
